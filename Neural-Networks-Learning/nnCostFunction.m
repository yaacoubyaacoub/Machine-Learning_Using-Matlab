...
a_2 = sigmoid([ones(size(X,1),1) X]*Theta1');

h_theta = sigmoid([ones(size(a_2,1),1) a_2]*Theta2');

J = ((1/m)*sum(sum(((-temp_y).*log(h_theta))-((1-temp_y).*log(1-h_theta)))))+...
...
for t=1:m
    ...
    z2 = [1, a1]*Theta1';   % 1x25
    a2 = sigmoid(z2);  %1x25
    a3 = sigmoid([1, a2]*Theta2');   %1x10
    
    delta3 = (a3 - temp_y(t,:))';   %10x1
    delta2 = (Theta2'*delta3).*sigmoidGradient([1 z2]');  %26x1
    
    Theta2_grad = Theta2_grad + delta3*[1 a2];  %10x26
    Theta1_grad = Theta1_grad + delta2(2:end,1)*[1 a1];  %25x401
end
